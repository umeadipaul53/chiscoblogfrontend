...
const VueJs = () => {
  ...
  const posts =
    useSelector((state) => state.post.postsByCategory?.VueJs?.data) || [];

  const pagination =
    useSelector((state) => state.post.postsByCategory?.VueJs?.pagination) || {};

  const count =
    useSelector((state) => state.post.postsByCategory?.VueJs?.count) || 0;

  const [page, setPage] = useState(1);
  const category = "VueJs";
  ...
  return (
    <section className="business-page">
      {/* Header */}
      <h1 className="page-title">VueJs Posts ({count})</h1>
